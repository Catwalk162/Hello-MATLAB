...
matriz = round(10 * randn(M,M));
det_anterior = det(matriz);
tam = size(matriz);
k0 = rank(matriz);
f = randi([1, M]);
...
det_final = det(matriz);

disp('Rank anterior = '); disp(k0);
disp('Determinante anterior = '); disp(det_anterior);

disp('Rank final = '); disp(k1);
disp('Determinante final = '); disp(det_final);
